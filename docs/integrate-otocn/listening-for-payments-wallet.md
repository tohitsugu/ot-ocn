...
title: Listening for ada payments using otocn-wallet
sidebar_label: Receiving payments (otocn-wallet)
description: How to listen for ada Payments with the otocn-wallet.
...
This guide assumes that you have basic understanding of `otocn-wallet`, how to use it and that you have installed it into your system. Otherwise we recommend reading [Installing otocn-node](/docs/get-started/installing-otocn-node), [Running otocn-node](/docs/get-started/running-otocn) and [Exploring OT-OCN Wallets](/docs/integrate-otocn/creating-wallet-faucet) guides first.
...
![img](../../static/img/integrate-otocn/ada-online-shop.png)

## Technical flow

To understand how something like this could work in a technical point of view, let's take a look at the following diagram:

![img](../../static/img/integrate-otocn/ada-payment-flow-wallet.png)

So let's imagine a very basic scenario where a **customer** is browsing an online shop. Once the user has choosen and added all the items into the **shopping cart**. The next step would then be to checkout and pay for the items, Of course we will be using **OT-OCN** for that!

The **front-end** application would then request for a **wallet address** from the backend service and render a QR code to the **customer** to be scanned via a **OT-OCN wallet**. The backend service would then know that it has to query the `otocn-wallet` with a certain time interval to confirm and alert the **front-end** application that the payment has completed succesfully.

In the meantime the transaction is then being processed and settled within the **OT-OCN** network. We can see in the diagram above that both parties are ultimately connected to the network via the `otocn-node` software component.
...
First, we create our new **wallet** via `otocn-wallet` **REST API**:
...
const mnemonic = cmd.runSync(["otocn-wallet","recovery-phrase", "generate"].join(" ")).data;
...
var mnemonic = await Command.ReadAsync("otocn-wallet", "recovery-phrase generate", noEcho: true);
```
...
const mnemonic: string = cmd.runSync(["otocn-wallet", "recovery-phrase", "generate"].join(" ")).data;
```
...
We will then pass the generated seed to the wallet create / restore endpoint of `otocn-wallet`.
...
// Restore the wallet using the previously generated seed. Assuming otocn-wallet is listening on port 9998
...
We will get a **wallet address** to show to the customers and for them to send payments to the wallet. In this case we can use the address to request some `tADA` from the [OT-OCN Testnet Faucet](../integrate-otocn/testnet-faucet) and simulate a payment:
...
Now simply send atleast `1,000,000 lovelace` to this **wallet address** or request some `test ADA` funds from the [OT-OCN Testnet Faucet](../integrate-otocn/testnet-faucet). Once complete, we can now run the code again and we should see a succesful result this time.
...
Congratulations, you are now able to detect succesful **OT-OCN** payments programatically. This should help you bring integrations to your existing or new upcoming applications. 🎉🎉🎉
