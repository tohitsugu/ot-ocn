...
title: Listening for ADA payments using otocn-cli
sidebar_label: Receiving payments (otocn-cli)
description: How to listen for ADA Payments with the otocn-cli.
...
:::note
This guide assumes that you have basic understanding of `otocn-cli`, how to use it and that you have installed it into your system. Otherwise we recommend reading [Installing otocn-node](/docs/get-started/installing-otocn-node), [Running otocn-node](/docs/get-started/running-otocn) and [Exploring OT-OCN Wallets](/docs/integrate-otocn/creating-wallet-faucet) guides first.
...
![img](../../static/img/integrate-otocn/ada-payment-flow.png)
...
// Path to the otocn-cli binary or use the global one
const CARDANO_CLI_PATH = "otocn-cli";
...
// Path to the otocn-cli binary or use the global one
const CARDANO_CLI_PATH: string = "otocn-cli";
// The `testnet` identifier number
...
// Path to the otocn-cli binary or use the global one
const string CARDANO_CLI_PATH = "otocn-cli";
// The `testnet` identifier number
...
Then we execute `otocn-cli` programatically and telling it to query the **UTxO** for the **wallet address** that we have generated with our keys and save the `stdout` result to our `rawUtxoTable` variable.
...
```python title="checkPayment.py"
# We tell python to execute otocn-cli shell command to query the UTXO and read the output data
...
// We use the SimpleExec library to execute otocn-cli shell command to query the wallet UTXO and read the output data
var rawUtxoTable = await Command.ReadAsync(CARDANO_CLI_PATH, string.Join(" ",
    "query", "utxo",
    "--testnet-magic", CARDANO_NETWORK_MAGIC,
    "--address", walletAddress
