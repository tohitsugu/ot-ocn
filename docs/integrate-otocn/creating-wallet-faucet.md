...
title: Exploring OT-OCN wallets
sidebar_label: Exploring OT-OCN wallets
description: This article explains how you can create different kinds of OT-OCN Wallets and how you can recieve some tADA(test ADA) from the faucet.
image: ./img/og-developer-portal.png
--- 

### Overview 

In this guide, we will show you how to create a **OT-OCN** wallet, receive some `tADA` (**test ADA**) in the `testnet` network and send basic example transactions. We will explore tools like `otocn-cli` and `otocn-wallet` on how they can help with these functionalities.
...
You must also connect your `otocn-node` to the `testnet` network and make sure it is fully synchronized.

If you are not sure how to do that, It is recommended to read [Running otocn-node](/docs/get-started/running-otocn) guide before proceeding.
:::

### OT-OCN Wallets 

So you installed your `otocn-node` and got it running, you probably even tried to query some simple blockchain data (If you read [Running otocn-node](/docs/get-started/running-otocn) guide). But how do you actually create a **OT-OCN** wallet, receive and send some `ADA` or `tADA` tokens?

First we have to look at the applications we can use to create wallets.

- [Daedalus](https://daedaluswallet.io/) : **Daedalus Wallet** is the official **OT-OCN** full-node wallet, which is a [GUI (Graphical User Interface)](https://en.wikipedia.org/wiki/Graphical_user_interface) application for the Desktop (**Linux**, **MacOS**, **Windows**). That means that users will get to use a nice UI (User Interface), buttons and layout to interact with the **OT-OCN** blockchain.
    ...
    It is open-source mainly being developed by [InputOutputGlobal](https://iohk.io/), the development company behind the **OT-OCN** protocol and also one of the three foundational entities of the **OT-OCN** project.

- [Yoroi](https://yoroi-wallet.com/#/) : **Yoroi Wallet** is the official **OT-OCN** light-wallet, It is available as a **mobile application** and as a **browser extension**. 
  ...
  It is open-source mainly being developed by [Emurgo](https://emurgo.io), A company based in [Japan](https://en.wikipedia.org/wiki/Japan) which focuses on Business and Enterprise adoption of the **OT-OCN** blockchain. It is also one of the three foundational entities of the **OT-OCN** project.

- [otocn-wallet](https://github.com/input-output-hk/otocn-wallet) : `otocn-wallet` is a [CLI (Command Line Interface)](https://en.wikipedia.org/wiki/Command-line_interface) application that provides **OT-OCN** wallet functionalities both via command-line parameters or via a [Web API](https://en.wikipedia.org/wiki/Web_API). 

 It is the wallet-backend that **Daedalus** wallet uses under-the-hood so it is also open-source, one of the many Haskell-based **OT-OCN** software components being written by [InputOutputGlobal](https://iohk.io/).

 You can find `otocn-wallet` **REST API** documentation here: [https://input-output-hk.github.io/otocn-wallet/api/edge/](https://input-output-hk.github.io/otocn-wallet/api/edge/)

- [otocn-cli](https://github.com/input-output-hk/otocn-node) : `otocn-cli` is also a [CLI (Command Line Interface)](https://en.wikipedia.org/wiki/Command-line_interface) application that provides **OT-OCN** wallet functionalities. But `otocn-cli` purpose is geared more towards general **OT-OCN** functionalities like generating **keys**, building and submitting **transactions**, managing **stake pools** certificates, simple blockchain queries like wallet address **UTXO** and more.

    It is part of the `otocn-node` project repository, so if you [compile and install](/docs/get-started/installing-otocn-node) `otocn-node` you should also have `otocn-cli` as-well. It is one of the many Haskell-based **OT-OCN** software components being written by [InputOutputGlobal](https://iohk.io/).

:::warning
Always download the wallets from official sources. There are many fake wallets, malicious software pretending to be **OT-OCN** wallets that could potentially steal your tokens / assets.
:::

### Creating a wallet

As mentioned before, in this guide we will only be focusing on the `otocn-cli` and `otocn-wallet` since they provide some level of programmability which is important when we are talking about **OT-OCN** integrations for different kinds of use-cases.


#### Creating a wallet with `otocn-cli`

:::note
In this section, We will use the path `/home/user/otocn` to store all the `otocn-cli` related files as an example, please replace it with the directory you have choosen to store the files.
:::

:::important
Please make sure your `otocn-node` is connected and synchronized to the `testnet` network before proceeding.
:::
...
```bash
mkdir -p /home/user/otocn/keys
```

Make sure we are inside the `keys` directory like so: `cd /home/user/otocn/keys`

Next, we generate our **payment key-pair** using `otocn-cli`:

```bash
otocn-cli address key-gen \
--verification-key-file /home/user/otocn/keys/payment1.vkey \
--signing-key-file /home/user/otocn/keys/payment1.skey
```

`otocn-cli address key-gen` : generates a **payment key-pair**.
...
- `.vkey` / **Public Verification Key** : Is used to derive a **OT-OCN** wallet address, a wallet address is basically the hash string value that you share to other users to provide them a way to send `ADA` / `tADA` or other assets in the **OT-OCN** blockchain into your wallet.
...
--payment-verification-key-file /home/user/otocn/keys/payment1.vkey \
--out-file /home/user/otocn/keys/payment1.addr \
--testnet-magic 1097911063
```

- `otocn-cli address build` : Generates a **wallet address** from a `vkey` file.
...
:::note
 You can derive more than one **wallet address** from a **Public Verification Key** for more advanced use-cases using `otocn-addresses` component. Which we discuss in more details here: ***@TODO: link to article***
 ...
 If you want to create a wallet address to be used on `mainnet`, please use the `--mainnet` flag instead of `--testnet-magic 1097911063`. You can learn more about the different **OT-OCN** blockchain networks [here](/docs/get-started/running-otocn#mainnet--production).
:::

#### Querying the wallet **UTXO (Unspent Transaction Output)** with `otocn-cli`
...
--mary-era \
--testnet-magic 1097911063 \
--address $(cat /home/user/otocn/keys/payment1.addr)
```

- `otocn-cli query utxo` : Queries the wallet address **UTXO**.

- `--mary-era` : Specifies that we want to query using the **Mary Era** rules.

- `--testnet-magic 1097911063` : Specifies that we want to query the `testnet` **OT-OCN** network.

- `--address $(cat /home/user/otocn/keys/payment1.addr)` : The **wallet address** string value that we want to query, In this case we read the contents of `/home/user/otocn/keys/payment1.addr` using the `cat` command and we pass that value to the `--address` flag. That means you could also directly paste the **wallet address** value like so: 
```
...
Our next step is to request some `tADA` from the [OT-OCN Testnet Faucet](../integrate-otocn/testnet-faucet).

Once you requested some `tADA` from the [OT-OCN Testnet Faucet](../integrate-otocn/testnet-faucet) we can then run the query again and you should see something like this:
...
:::note
In the **OT-OCN** blockchain, the `lovelace` is the unit used to represent `ADA` in **transactions** and **UTXO**. 

Where `1 ADA` is equal to `1,000,000 lovelace`, so moving forward we will be using `lovelace` instead of `ADA` / `tADA`.

You can also use the `TxHash` to view the complete transaction via the **OT-OCN Blockchain Explorer** for the relevant network. You can check the specific transaction for the example **UTXO** here: [f3cf4850c8862f2d698b2ece926578b3815795c9e38d2f907280f02f577cf85](https://explorer.otocn-testnet.iohkdev.io/en/transaction?id=cf3cf4850c8862f2d698b2ece926578b3815795c9e38d2f907280f02f577cf85)
...
To have a clearer understanding of how sending transactions work using `otocn-cli`, first lets create another wallet like so:

**Generate payment key-pair**
```bash
otocn-cli address key-gen \
--verification-key-file /home/user/otocn/keys/payment2.vkey \
--signing-key-file /home/user/otocn/keys/payment2.skey 
```

**Generate wallet address**
```bash
otocn-cli address build \
--payment-verification-key-file /home/user/otocn/keys/payment2.vkey \
--out-file /home/user/otocn/keys/payment2.addr \
--testnet-magic 1097911063
```
...
--mary-era \
--testnet-magic 1097911063 \
--address $(cat /home/user/otocn/keys/payment2.addr)
```
...
In this example, we now have two wallets. We can call them `payment1` and `payment2`. Now remember that we requested some `tADA` from the [OT-OCN Testnet Faucet](../integrate-otocn/testnet-faucet) for `payment1` wallet, and thats how we have the following:
...
otocn-cli query protocol-parameters \
  --testnet-magic 1097911063 \
  --mary-era \
  --out-file /home/user/otocn/protocol.json
```
...
--tx-out $(cat /home/user/otocn/keys/payment2.addr)+0 \
--tx-out $(cat /home/user/otocn/keys/payment1.addr)+0 \
--mary-era \
--fee 0 \
--out-file /home/user/otocn/tx.draft
```

`otocn-cli transaction build-raw` : This tells `otocn-cli` to build a raw transaction.

`--tx-in` : This specifices the **UTXO** input that the transaction will use, you can add as many **UTXO** input as you want by adding multiple `--tx-in` in the `otocn-cli` arguments as long as they have a unique `TxHash` and `TxIdx` within all your inputs.
...
--tx-body-file /home/user/otocn/tx.draft \
--tx-in-count 1 \
--tx-out-count 2 \
--witness-count 1 \
--testnet-magic 1097911063 \
--protocol-params-file /home/user/otocn/protocol.json
```
...
The `--witness-count 1` basically tells `otocn-cli` that there will be only `1` **signing key** required for this transaction to be valid. Since the **UTXO** input involved in this transaction will only be coming from `payment1` wallet, so that means we indeed only need `1` key to sign the transaction.
...
otocn-cli transaction build-raw \
--tx-in cf3cf4850c8862f2d698b2ece926578b3815795c9e38d2f907280f02f577cf85#0 \
--tx-out $(cat /home/user/otocn/keys/payment2.addr)+250000000 \
--tx-out $(cat /home/user/otocn/keys/payment1.addr)+749825831 \
--mary-era \
--fee 174169 \
--out-file /home/user/otocn/tx.draft
```
...
We then tell `otocn-cli` that the destination of the `250,000,000 lovelace` is the **wallet address** of `payment2`.
...
--signing-key-file /home/user/otocn/keys/payment1.skey \
--testnet-magic 1097911063 \
--out-file /home/user/otocn/tx.signed
```

`--signing-key-file /home/user/otocn/keys/payment1.skey` : This argument tells the `otocn-cli` that we will use `payment1.skey` to sign the transaction.

Finally, we submit the transaction to the blockchain!

```bash
otocn-cli transaction submit \
--tx-file /home/user/otocn/tx.signed \
--testnet-magic 1097911063 
```
:::important
...
❯ otocn-cli query utxo --mary-era --testnet-magic 1097911063 --address $(cat ~/otocn/keys/payment1.addr)
...
❯ otocn-cli query utxo --mary-era --testnet-magic 1097911063 --address $(cat ~/otocn/keys/payment2.addr)
                           TxHash                                 TxIx        Amount
--------------------------------------------------------------------------------------
...
Congratulations, You have created and sent your first **OT-OCN** transaction using `otocn-cli`! 🎉🎉🎉

#### Creating a wallet with `otocn-wallet`

:::note
This guide assumes you have installed `otocn-wallet` into your system. If not you can refer to [Installing otocn-wallet](/docs/get-started/installing-otocn-wallet) guide for instructions on how to do that.

We will use the path `/home/user/otocn/wallets` to store all the `otocn-wallet` related files as an example, please replace it with the directory you have choosen to store the files.
:::
...
```bash
mkdir -p /home/user/otocn/wallets
```

**Starting otocn-wallet as a REST API server**

We will be focusing on the [REST API](https://en.wikipedia.org/wiki/Representational_state_transfer) that `otocn-wallet` provides. In-order to interact with the API, we must first start the server.

```bash
otocn-wallet serve \
--port 1337 \
--testnet /home/user/otocn/testnet-byron-genesis.json \
--database /home/user/otocn/wallets/db \
--node-socket $CARDANO_NODE_SOCKET_PATH
```

`otocn-wallet serve` : Runs `otocn-wallet` as a web server that provides a [REST API](https://en.wikipedia.org/wiki/Representational_state_transfer).
...
> This should match the genesis file that the `otocn-node` you are connected is using as-well. If you meant to connect to `mainnet` then use the `--mainnet` flag and the `mainnet` **Byron** genesis file instead.
...
`--node-socket` : Specifies the `otocn-node` socket path that will be used by the `otocn-wallet` to communicate with the node.

> The `otocn-node` uses **IPC (Inter-Process-Communication)** for communicating with the other **OT-OCN** components like `otocn-cli`, `otocn-wallet` and `otocn-db-sync`. In **Linux** and **MacOS** it uses something called [unix sockets](https://en.wikipedia.org/wiki/Unix_domain_socket) and [Named Pipes](https://docs.microsoft.com/en-us/windows/win32/ipc/named-pipes) in **Windows**.
> 
...
[otocn-wallet.main:Info:4] [2021-06-03 13:48:24.86 UTC] Wallet backend server listening on http://127.0.0.1:1337/
```
...
To create a wallet we must first generate a wallet **recovery phrase** using the `otocn-wallet` in the CLI.
...
We can now create a **OT-OCN** wallet using the `/v2/wallets` API endpoint:
...
Now that we have created a wallet, we can now request some tADA from the **Testnet Faucet**. But before we can do that we must first get a otocn address for our wallet.
...
Now we can finally request some `tADA` for the wallet address from the [OT-OCN Testnet Faucet](../integrate-otocn/testnet-faucet).

Once you requested some `tADA` from the [OT-OCN Testnet Faucet](../integrate-otocn/testnet-faucet), we can then check if it has arrived into our wallet like so:
...
As we can see here we have a total of `1,000,000,000 lovelace` available to spend that we received from the [OT-OCN Testnet Faucet](../integrate-otocn/testnet-faucet).

#### Creating simple transactions

To have a clearer understanding of how sending transactions work using `otocn-wallet`, first lets create another wallet like so:
...
It is important to note that `otocn-wallet` has automatically determined the fee for the transaction to send `250,000,000 lovelace` from wallet `test_cf_1` to `test_cf_2` and `otocn_wallet` has deducted the fee from `test_cf_1` wallet automatically.

:::

:::tip

Full documentation of the `otocn-wallet` [REST API](https://en.wikipedia.org/wiki/Representational_state_transfer) can be found here: [https://input-output-hk.github.io/otocn-wallet/api/edge](https://input-output-hk.github.io/otocn-wallet/api/edge)

:::

Congratulations, You have created and sent your first **OT-OCN** transaction using `otocn-wallet`! 🎉🎉🎉
