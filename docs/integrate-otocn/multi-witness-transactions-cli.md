...
--tx-out $(cat ~/otocn/keys/payment1.addr)+0 \
--fee 0 \
--out-file ~/otocn/multi-witness-sample/tx-single1.draft

otocn-cli transaction build-raw \
--tx-in 258abd628eef7d6ff0f7b4e6866b4f7c21065f4d6b5e49b51e2ac4ff035ad06f#0 \
--tx-out $(cat ~/otocn/keys/payment2.addr)+0 \
--fee 0 \
--out-file ~/otocn/multi-witness-sample/tx-single2.draft
```

And invoke the calculate-min-fees endpoint on `otocn-cli` for both of them:

```bash {8,17}
otocn-cli transaction calculate-min-fee \
--tx-body-file ~/otocn/multi-witness-sample/tx-single1.draft \
--tx-in-count 1 \
--tx-out-count 1 \
--witness-count 1 \
--testnet-magic 1097911063 \
--protocol-params-file ~/otocn/protocol.json 
169857 Lovelace

otocn-cli transaction calculate-min-fee \
--tx-body-file ~/otocn/multi-witness-sample/tx-single2.draft \
--tx-in-count 1 \
--tx-out-count 1 \
--witness-count 1 \
--testnet-magic 1097911063 \
