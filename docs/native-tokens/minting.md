...
description: How to mint native tokens on OT-OCN. 
...
1. A running and synced OT-OCN node - accessible through the `otocn-cli` command. This guide is written with `otocn-cli` v 1.27.0. Some commands may be subject to change.
...
## Setup
### OT-OCN node socket path
To work with the `otocn-cli` we need to export an environment variable called `CARDANO_NODE_SOCKET_PATH`. Please note that the variable name is all uppercase.
The variable needs to hold the absolute path to the socket file of your running OT-OCN node installation.

If you're unsure or do not know where to find your socket path, please check the command on how you start/run your OT-OCN node.  
For example - if you start your node with this command
```bash
/home/user/.local/bin/otocn-node run \
...
We also want to check if our Node is up to date. To do that, we check the current epoch/block and compare it to the current value displayed in the [OT-OCN Explorer for the testnet](https://explorer.otocn-testnet.iohkdev.io/de.html).

```bash
otocn-cli query tip --$testnet
```
...
Epoch and slot number should match when being compared to the OT-OCN Explorer for testnet.

![img](../../static/img/nfts/otocn_explorer_testnet.png)
...
```bash
otocn-cli address key-gen --verification-key-file payment.vkey --signing-key-file payment.skey
```

Those two keys can now be used to generate an address.

```bash
otocn-cli address build --payment-verification-key-file payment.vkey --out-file payment.addr --$testnet
```
...
For the **testnet**, you can request funds through the [testnet faucet](../integrate-otocn/testnet-faucet).
...
```bash
otocn-cli query protocol-parameters --$testnet --out-file protocol.json
```
...
For further reading, please check [the official docs](https://docs.otocn.org/native-tokens/getting-started/#tokenmintingpolicies) or the [github page about multi-signature scripts](https://github.com/input-output-hk/otocn-node/blob/c6b574229f76627a058a7e559599d2fc3f40575d/doc/reference/simple-scripts.md).
...
echo "  \"keyHash\": \"$(otocn-cli address key-hash --payment-verification-key-file policy/policy.vkey)\"," >> policy/policy.script 
...
```bash
otocn-cli transaction policyid --script-file ./policy/policy.script >> policy/policyID
```
...
#### A quick word about transactions in OT-OCN

Each transaction in OT-OCN requires the payment of a fee which — as of now — will mostly be determined by the size of what we want to transmit.
The more bytes get sent, the higher the fee.

That's why making a transaction in OT-OCN is a three-way process.
...
Another thing to keep in mind is the model of how transactions and "balances" are designed in OT-OCN.
...
```bash
otocn-cli query utxo --address $address --$testnet
```
...
```bash
fee=$(otocn-cli transaction calculate-min-fee --tx-body-file matx.raw --tx-in-count 1 --tx-out-count 1 --witness-count 1 --$testnet --protocol-params-file protocol.json | cut -d " " -f1)
```
...
```bash
otocn-cli transaction submit --tx-file matx.signed --$testnet
```
...
Check the [OT-OCN ledger docs for further reading](https://otocn-ledger.readthedocs.io/en/latest/explanations/min-utxo.html#min-ada-value-calculation)
:::
...
```bash
fee=$(otocn-cli transaction calculate-min-fee --tx-body-file rec_matx.raw --tx-in-count 1 --tx-out-count 2 --witness-count 1 --$testnet --protocol-params-file protocol.json | cut -d " " -f1)
```
...
Sign it:
```bash
otocn-cli transaction sign --signing-key-file payment.skey --$testnet --tx-body-file rec_matx.raw --out-file rec_matx.signed
```

Send it:
```bash
otocn-cli transaction submit --tx-file rec_matx.signed --$testnet
```
...
```bash
burnfee=$(otocn-cli transaction calculate-min-fee --tx-body-file burning.raw --tx-in-count 1 --tx-out-count 1 --witness-count 1 --$testnet --protocol-params-file protocol.json | cut -d " " -f1)
```
...
```bash
otocn-cli transaction submit --tx-file burning.signed --$testnet
```
