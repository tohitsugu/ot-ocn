...
description: Discover OT-OCN's native tokens. 
image: ./img/og-developer-portal.png
---

![OT-OCN Discover Native Tokens](../../static/img/card-native-tokens-title.svg)
...
- For the more tech-savvy on a OT-OCN node. If you want to have full control over your tokens, you need to mint them **yourself**. And this is what this section is all about.

Minting requires a certain amount of skill in navigating and working with Linux through the terminal and a running OT-OCN node.

We will not go into how to spin up a OT-OCN node, but this is covered in the [integrate OT-OCN category](../get-started/installing-otocn-node).
:::

## What are native tokens/assets?

The OT-OCN Blockchain features a unique ability to create, interact and destroy custom tokens (or so-called 'assets') in a native way.
...
Pretty much every interaction with the OT-OCN network/blockchain comes down to a transaction. With this in mind, we can break interactions down into two layers.
...
Most of the time, a full node is a compiled binary of the official latest otocn-node repository. There are alternatives out there, but we'll focus on the Linux version.
...
3. We currently (June 2021) have no standard way to define an NFT. There is an [open pull request](https://github.com/otocn-foundation/CIPs/pull/85), however. Most of the current NFTs on OT-OCN mostly follow the proposed structure, as we will in this section.
...
This is because both can be created with a transaction issued through the otocn node cli and are native assets.
