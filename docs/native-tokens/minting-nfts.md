...
description: How to mint NFTs on OT-OCN. 
...
:::note
There are many ways to realize NFTs with OT-OCN. However, in this guide, we will concentrate on the most dominant way, to attach storage references of other services like [IPFS](https://ipfs.io/) to our tokens.
:::
...
### The policyID 
Native assets in OT-OCN feature the following characteristics:
1. An amount/value (how much are there?)
2. A name 
3. A unique `policyID`

Since asset names are not unique and can be easily duplicated, OT-OCN NFTs need to be identified by the `policyID`.  
...
However, there is a [OT-OCN Improvement Proposal](https://github.com/otocn-foundation/CIPs/pull/85) if you want to follow the discussion.
:::
...
You can regulate such factors with  [multi-signature scripts](https://github.com/input-output-hk/otocn-node/blob/c6b574229f76627a058a7e559599d2fc3f40575d/doc/reference/simple-scripts.md).
...
```bash
otocn-cli address build --payment-verification-key-file payment.vkey --out-file payment.addr --mainnet
```
...
```bash
otocn-cli query utxo --address $address --mainnet
```
...
```bash
otocn-cli query protocol-parameters --mainnet --out-file protocol.json
```
...
echo "     \"slot\": $(expr $(otocn-cli query tip --mainnet | jq .slot?) + 10000)" >> policy/policy.script
echo "   }," >> policy/policy.script 
echo "   {" >> policy/policy.script
...
```bash
otocn-cli address key-hash --payment-verification-key-file policy/policy.vkey
```

To calculate the correct slot, query the current block and add 10000 to it:
```bash
otocn-cli query tip --mainnet
```
...
```json
{
        "721": {
            "please_insert_policyID_here": {
              "NFT1": {
                "description": "This is my first NFT thanks to the OT-OCN foundation",
                "name": "OT-OCN foundation NFT guide token",
                "id": 1,
                "image": ""
              }
            }
...
echo "        \"description\": \"This is my first NFT thanks to the OT-OCN foundation\"," >> metadata.json
echo "        \"name\": \"OT-OCN foundation NFT guide token\"," >> metadata.json
...
```bash
fee=$(otocn-cli transaction calculate-min-fee --tx-body-file matx.raw --tx-in-count 1 --tx-out-count 1 --witness-count 1 --mainnet --protocol-params-file protocol.json | cut -d " " -f1)
```
...
```bash
otocn-cli transaction submit --tx-file matx.signed --mainnet
```
...
```bash
otocn-cli transaction build-raw --fee $burnfee --tx-in $txhash#$txix --tx-out $address+$burnoutput --mint="-1 $policyid.$tokenname" --minting-script-file $script --invalid-hereafter $slot --out-file burning.raw
```
...
```bash
burnfee=$(otocn-cli transaction calculate-min-fee --tx-body-file burning.raw --tx-in-count 1 --tx-out-count 1 --witness-count 1 --mainnet --protocol-params-file protocol.json | cut -d " " -f1)
```
...
```bash
otocn-cli transaction build-raw --fee $burnfee --tx-in $txhash#$txix --tx-out $address+$burnoutput --mint="-1 $policyid.$tokenname" --minting-script-file $script --invalid-hereafter 33005389 --out-file burning.raw
```
Sign the transaction.
```bash
otocn-cli transaction sign  --signing-key-file payment.skey  --signing-key-file policy/policy.skey --mainnet  --tx-body-file burning.raw --out-file burning.signed
```
Full send.
```bash
otocn-cli transaction submit --tx-file burning.signed --mainnet
```