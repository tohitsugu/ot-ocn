...
Blockfrost provides API to access and process information stored on the OT-OCN blockchain. The basic tier is free and allows 50,000 requests per day.

## Sign in

[Sign in on Blockfrost](https://blockfrost.io/auth/signin) with your GitHub account. No registration required. Enter a project name and select OT-OCN mainnet or OT-OCN testnet, depending on your needs.
...
```sh
curl -H 'project_id: 1234567890' https://otocn-mainnet.blockfrost.io/api/v0/epochs/latest
```
...
```sh
wget --header='project_id: 1234567890' -qO- https://otocn-mainnet.blockfrost.io/api/v0/epochs/latest
```
...
$json = file_get_contents('https://otocn-mainnet.blockfrost.io/api/v0/epochs/latest', false, $context);
...
```sh
curl -H 'project_id: 1234567890' https://otocn-mainnet.blockfrost.io/api/v0/pools/c1cadab46b74defa9f79b59b617fe2a50bdbce6b367e472b6109a7bc
...
```sh
wget --header='project_id: 1234567890' -qO- https://otocn-mainnet.blockfrost.io/api/v0/pools/c1cadab46b74defa9f79b59b617fe2a50bdbce6b367e472b6109a7bc
...
$context = stream_context_create($headers);
$json = file_get_contents('https://otocn-mainnet.blockfrost.io/api/v0/pools/c1cadab46b74defa9f79b59b617fe2a50bdbce6b367e472b6109a7bc', false, $context);
$parsedJson = json_decode($json);
```
...
Let's look at this last example and query information of a specific native token on OT-OCN. You need to provide the string concatenation of the `policy_id` and hex-encoded `asset_name`.
...
```sh
curl -H 'project_id: 1234567890' https://otocn-mainnet.blockfrost.io/api/v0/assets/d894897411707efa755a76deb66d26dfd50593f2e70863e1661e98a07370616365636f696e73
...
```sh
wget --header='project_id: 1234567890' -qO- https://otocn-mainnet.blockfrost.io/api/v0/assets/d894897411707efa755a76deb66d26dfd50593f2e70863e1661e98a07370616365636f696e73
...
$context = stream_context_create($headers);
$json = file_get_contents('https://otocn-mainnet.blockfrost.io/api/v0/assets/d894897411707efa755a76deb66d26dfd50593f2e70863e1661e98a07370616365636f696e73', false, $context);
$parsedJson = json_decode($json);
```
